  ...
  public async getEvents(
    ...
    let query = db("events")
      ...
      .leftJoin("rsvps as current_user_rsvp", function () {
        this.on("events.id", "=", "current_user_rsvp.event_id").andOn(
          "current_user_rsvp.user_id",
          "=",
          db.raw("?", [userId || 0]),
        );
      })
      ...
      .select(
        ...
        db.raw(
          `SUM(CASE WHEN all_rsvps.status = 'yes' THEN 1 ELSE 0 END) as rsvpYesCount`,
        ),
        db.raw(
          `SUM(CASE WHEN all_rsvps.status = 'no' THEN 1 ELSE 0 END) as rsvpNoCount`,
        ),
        db.raw(
          `SUM(CASE WHEN all_rsvps.status = 'maybe' THEN 1 ELSE 0 END) as rsvpMaybeCount`,
        ),
      )
